  ...
  generate(
    ...
    manage_bucket_script(bucket, obj)::
      ...
      local createKeyCmd = if std.get(obj, 'encrypt', false) then
        'curl -v -XPOST -d "enclave=%s" https://${MINIO_ENDPOINT}:7373/v1/key/create/%s --cert %s --key %s --cacert %s' % [namespace, bucket, config.minioKesClientCertPath, config.minioKesClientKeyPath, config.kesRootCAPath] else '';
