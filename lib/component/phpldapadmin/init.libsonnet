  ...
  generate(
    ...
    configmap: kube.ConfigMap(componentName) {
      ...
      data: {
        ...
        PHPLDAPADMIN_LDAP_HOSTS: "#PYTHON2BASH:[{'%s': [{'server': [{'base': ['o=auth,dc=local']},{'tls': True},{'port':389}]},{'login': [{'bind_id': 'cn=%s,%s' }]}]}]" % [config.ldapSvc, config.ldapAdmin, config.ldapRoot],
