...
def sanitize_obj(doc):
    ...
    doc_id = "{}_{}: kube.{}(componentName)".format(
        str.lower(kind), doc["metadata"]["name"].replace("-", "_"), kind
    )
    ...
    try:
        if doc["spec"]["volumeClaimTemplates"]:
            for i, _ in enumerate(doc["spec"]["volumeClaimTemplates"]):
                doc["spec"]["volumeClaimTemplates"][i]["metadata"][
                    "labels"
                ] = "config.labels"
    except KeyError:
        pass
    ...
    for i, line in enumerate(data.splitlines(), start=1):
        ...
        if ": kube." in line:
            ...
            lines.append(
                line.replace(
                    "image:",
                    "image: helper.getImage(config.imageRegistry, config.imageRef, config.imageVersion), // orig:",
                )
            )
        else:
            lines.append(line)

    # add merge operator to usual fields
    lines2 = []
    for i, line in enumerate(lines, start=1):
        if "metadata:" in line:
            lines2.append(line.replace("metadata:", "metadata+:"))
        elif "labels:" in line:
            lines2.append(line.replace("labels:", "labels+:"))
        elif "annotations:" in line:
            lines2.append(line.replace("annotations:", "annotations+:"))
        elif "spec:" in line:
            lines2.append(line.replace("spec:", "spec+:"))
        elif "template:" in line:
            lines2.append(line.replace("template:", "template+:"))
        else:
            lines2.append(line)

    template_end = ["}),", "}"]

    return "\n".join(template_start + lines2 + template_end)
...
def main(argv):
    ...
    try:
        ...
        if (
            os.path.exists(_from)
            and not os.path.exists(_to_res)
            and not os.path.exists(_to_sec)
        ):
            ...
            log.error(
                f"source file {_from} does not exist or dest file(s) {_to_res}, {_to_sec} does exist (no overwriting)."
            )
    except IndexError:
